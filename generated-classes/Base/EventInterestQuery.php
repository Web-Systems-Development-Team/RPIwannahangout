 ...
 *
 * @method     ChildEventInterestQuery leftJoinTarget_Event($relationAlias = null) Adds a LEFT JOIN clause to the query using the Target_Event relation
 * @method     ChildEventInterestQuery rightJoinTarget_Event($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Target_Event relation
 * @method     ChildEventInterestQuery innerJoinTarget_Event($relationAlias = null) Adds a INNER JOIN clause to the query using the Target_Event relation
 *
...
abstract class EventInterestQuery extends ModelCriteria
{
    ...
    public function filterByTargetEventId($targetEventId = null, $comparison = null)
    {
        if (is_array($targetEventId)) {
            ...
            if (isset($targetEventId['min'])) {
                $this->addUsingAlias(EventInterestTableMap::COL_TARGET_EVENT_ID, $targetEventId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($targetEventId['max'])) {
                $this->addUsingAlias(EventInterestTableMap::COL_TARGET_EVENT_ID, $targetEventId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(EventInterestTableMap::COL_TARGET_EVENT_ID, $targetEventId, $comparison);
    }
    ...
    public function filterByTarget_Event($event, $comparison = null)
    {
        if ($event instanceof \Event) {
            return $this
                ->addUsingAlias(EventInterestTableMap::COL_TARGET_EVENT_ID, $event->getEventId(), $comparison);
            ...
            return $this
                ->addUsingAlias(EventInterestTableMap::COL_TARGET_EVENT_ID, $event->toKeyValue('PrimaryKey', 'EventId'), $comparison);
        } else {
